...
export class CreateVentasComponent implements OnInit {
  ...
  select_cliente(item: any) {
    this.venta.cliente = item.id;
    $('#modalCliente').modal('hide');
    $('#input-cliente').val(item.nombres + ' ' + item.apellidos);
    this.init_direcciones(item.id);
  }
      ...
      response => {
        ...
        response.data.forEach((element: any) => {
          this.variedades.push({
            ...
            precioDolar: element.producto.precioDolar,
      ...
      if (this.cantidad >= 1) {
        if (this.cantidad <= this.producto_select.cantidad) {
          this.dventa.push({
            ...
            precio_und: this.producto_select.precio,
            titulo_variedad: this.producto_select.variedad,
            producto: this.producto_select.idproducto,
            subtotal: this.producto_select.precio * this.cantidad,
            variedad: this.producto_select.idvariedad,
            cantidad: this.cantidad
          });
          console.log(this.dventa);
          this.total_pagar = this.total_pagar + (this.producto_select.precio * this.cantidad);
          this.neto_pagar = this.neto_pagar + (this.producto_select.precio * this.cantidad);
        } else {
          iziToast.show({
            title: 'ERROR',
            titleColor: '#FF0000',
