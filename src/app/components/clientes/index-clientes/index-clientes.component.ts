...
export class IndexClientesComponent implements OnInit {
  ...
  filtrar_cliente() {
    if (this.filtro) {
      ...
      this.clientes = this.clientes_const.filter(item => term.test(item.nombres) || term.test(item.apellidos) || term.test(item.email) || term.test(item.dni) || term.test(item.telefono) || term.test(item.id));
    } else {
      this.clientes = this.clientes_const;
    }
  }
